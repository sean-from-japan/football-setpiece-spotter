...
**ピッチの内側で当てはめた多項式は、その外側について意味のあることを何も言いません。** しかも静かに間違えるのではなく派手に間違えます。実写での最初の実行では、フレーム上方の観客や樹木を写した結果が数百メートル先の座標になりました。そのため `on_pitch()` が、タッチラインの外側12 mを超える座標を、人数を数える処理に届く前に捨てます。
